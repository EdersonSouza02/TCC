...
public class ClienteDAO {
	...
	public void cadastraCliente(ClienteVO cVO) throws Exception {
		...
		PreparedStatement preparedStatement1 = this.connection.prepareStatement(
				"INSERT INTO Endereco(Rua_Endereco,Bairro_Endereco,Numero_Endereco,Cep_Endereco,Cidade_Endereco,UF_Endereco,Complemento_Endereco, Codigo_Cliente)VALUES(?,?,?,?,?,?,?,?)");
		...
		preparedStatement1.setString(6, cVO.getUf());
		preparedStatement1.setString(7, cVO.getComplemento());
		preparedStatement1.setInt(8, cVO.getCodigo());
	...
	public String excluirCliente(int codigo) throws SQLException {
		String retorno = "Não foi possivel excluir o Cliente";
	...
	public ClienteVO pesquisarContatoTodos(int codigo) throws SQLException {
		...
		if (resultSet1.next() && resultSet2.next() && resultSet3.next()) {
			...
			clienteVO.setUf(resultSet2.getString("UF_Endereco"));
	...
	public void alteraCliente(ClienteVO clienteVO) throws SQLException {
		...
		PreparedStatement preparedStatement1 = this.connection.prepareStatement(
				"UPDATE Endereco SET Rua_Endereco = ?, Bairro_Endereco = ?, Numero_Endereco = ?, Cep_Endereco = ?, Cidade_Endereco = ?,UF_Endereco =?,Complemento_Endereco=? Where Codigo_Cliente = ?;");
		PreparedStatement preparedStatement2 = this.connection.prepareStatement(
				"UPDATE Contato SET Cel_Contato = ?, Fixo_Contato = ?, Nextel_Contato = ? Where Codigo_Cliente = ?;");
		PreparedStatement preparedStatement3 = this.connection.prepareStatement(
				"UPDATE Cliente SET Nome_Cliente = ?, CNPJ_Cliente = ?, CPF_Cliente = ?, Forma_De_Pagamento_Cliente = ? Where Codigo_Cliente = ?;");
		...
		preparedStatement1.setString(6, clienteVO.getUf());
		preparedStatement1.setString(7, clienteVO.getComplemento());
		preparedStatement1.setInt(8, clienteVO.getCodigo());
	...
	public List<ClienteVO> pesquisarNome(String nome) throws Exception {
		...
		PreparedStatement preparedStatement = this.connection
				.prepareStatement("Select * from Cliente  where Nome_Cliente like '%" + nome + "%'");
	...
	public boolean verificaExistenciaCodigo_Cliente(int codigo) throws Exception {
		PreparedStatement preparedStatement = this.connection
				.prepareStatement("select Codigo_Cliente from cliente where Codigo_Cliente=?");
